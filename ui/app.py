...
def _render_target_profile(profile: Any) -> None:
    ...
    with col_pos:
        ...
        data = [item.model_dump(mode="json") for item in profile.positive_deltas]
        st.table(data or [{"metric": "-", "label": "-", "value": None}])
    with col_neg:
        st.markdown("**Negative Deltas**")
        data = [item.model_dump(mode="json") for item in profile.negative_deltas]
        st.table(data or [{"metric": "-", "label": "-", "value": None}])

    st.markdown("**Top Risks**")
    data = [item.model_dump(mode="json") for item in profile.top_risks]
    st.table(data or [{"signal_type": "-", "severity": "-", "section": None, "summary": None, "occurrences": 0}])
...
def _render_report(artifact: OrchestrationArtifact, *, artifact_path: Optional[str] = None) -> None:
    ...
    if bundle.matches:
        st.table([m.model_dump(mode="json") for m in bundle.matches])
    else:
        st.info("No peer matches available.")

    if report.peer_snapshot:
        with st.expander("Peer Snapshot", expanded=True):
    ...
    st.table(
        [r.model_dump(mode="json") for r in report.risk_overlap_rows]
        or [{"group": "-", "risk_types": []}]
    )
